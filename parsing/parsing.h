...
/*   Updated: 2024/12/10 14:44:36 by gtraiman         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void							init_signals_and_env(t_env **env_list,
									char **envp);
...
void							cleanup_resources(t_data *data,
									t_env **env_list, t_cmd_list *list);
char							*extract_variable_value(char *input, int *i,
									t_env *env_list);
char							**xtrct_split_var_val(char *inp, int *i,
									t_env *e_l, char *dol_seq);
char							*bild_cmbined_val(int dol_cnt, char *dol_seq,
									char *var_val);
int								append_special_chars(char *input, int i,
									char **combined_value);
void							free_split_values(char **split_values);
int								handle_quotes_in_var(char *inp, int i,
									char **token_value);
char							*get_token_value(t_ctx *ctx,
									char **split_values, int j);
int								process_first_token(t_ctx *ctx, int i,
									char **token_value);
void							finalize_first_token(t_ctx *ctx);
int								handle_first_token(t_ctx *ctx, int i,
									char **token_value);
void							handle_standard_token(t_ctx *ctx,
									char *token_value);
void							finalize_token_if_first(t_ctx *ctx, int j);
int								handle_tok_proc(t_ctx *ctx, char **split_values,
									int j, int i);
int								add_all_tokens(t_ctx *ctx, char **split_values,
									int i);
int								gere_valid_var(t_ctx *ctx);
int								cree_et_add_tok_lit(t_ctx *ctx, int start,
									int end);
...
t_cmd_list						*init_command_node(t_cmd_list **cmd_list,
									t_cmd_list **current_cmd);
void							post_process_command(t_cmd_list *current_cmd,
									t_env **env_list);
int								p_singl_c(t_cmd_list **cmd_l, t_cmd_list **c_c,
									t_token **tok, t_env **e_l);
t_cmd_list						*parse_commands(t_token *tokens,
									t_env **env_list);
int								allocate_command_args(t_cmd_list *current_cmd,
									int arg_count);
...
int								handle_exit_status(t_env *env_list,
									char **result);
int								append_character_main_and_check(char **result,
									char c);
...
int								update_cmd_files(t_cmd_list *curr_cmd,
									char *temp_filename);
...
void							extract_name_value(char *env_var, char **name,
									char **value);
t_env							*create_env_node(char *name, char *value);
t_env							*init_env(char **envp);
void							free_cmd_args(char **cmd_args);
void							free_files_list(char **files_list,
									int files_count);
...
char							*append_variable_value(char *value,
									t_var_exp *ctx);
char							*process_invalid_variable(t_var_exp *ctx,
									char *value);
char							*process_exit_status(t_var_exp *ctx,
									char *value);
char							*process_dollar_sign(t_var_exp *ctx,
									char *value);
char							*handle_quoted_exit_status(t_env *env_list,
									int *index);
char							*process_regular_character(t_var_exp *ctx,
									char *value);
char							*process_double_quote_characters(t_var_exp *ctx,
									char *value);
char							*process_double_quote_content(t_var_exp *ctx);
int								init_var_exp_struc(t_var_exp *ctx,
									t_var_exp_params *params);
int								add_to_files_list(t_cmd_list *cmd,
									char *filename, int new_count);
int								add_to_files_type(t_cmd_list *cmd, int type,
									int new_count);
void							add_redirection(t_cmd_list *cmd, char *filename,
									int type);
...
int								handle_redirection_tokens(t_token **tokens,
									t_cmd_list *curr_cmd);
int								process_token_cmd(t_token **tokens,
									t_cmd_list *curr_cmd);
...
int								handle_word(char *input, int i,
									t_token **tokens, t_env *env_list);
int								gere_var_ref(char *inp, int i, t_token **tok,
									t_env *e_l);
int								handle_quotes(t_ctx *ctx);
char							*extract_quoted_content(char *input, int *i,
									char quote_type);
char							*xpnd_or_cp_cntnt(char *quoted_cntnt,
									char quote_type, t_env *e_l);
char							*safe_concat(char *final_content,
									char *new_content);
char							*gere_adjcnt_txt(char *input, int *i,
									char *final_content);
char							*process_quoted_section(t_quote_context *ctx,
									char quote_type);
...
char							*initialize_var_value(char *inp, int i,
									t_env *e_l);
char							*accumulate_invalid_chars(char *inp, int *i,
									char *token_value);
int								finalize_and_add_tok(t_token **tok,
									char *tok_val, char *dol_seq, int i);
int								handle_dollar_sequence(t_ctx *ctx);
int								gere_num_var(t_ctx *ctx);
int								accumulate_dol_seq(char *inp, int i,
									char **dol_seq, int *dol_cnt);
void							combine_prefix_with_var(t_pars_cxt *ctx,
...
t_pars_cxt						init_parsing_context(char *inp, int i,
									t_token **tok, t_env *env_list);
int								handle_solo_quotes(char *input, int i,
...
int								handle_character_iteration(char *input, int i);
char							*extract_and_expand(char *input, int *i,
									t_env *env_list);
...
int								handle_brackets_token(char *input, int i,
									t_token **tokens, t_env *env_list);
int								handle_backslash_token(char *input, int i,
									t_token **tokens);
...
int								extract_indices(char *input, int i,
									t_variable_data *data);
int								extract_name_and_val(char *inp,
									t_variable_data *data);
int								add_var_token_and_cleanup(t_variable_data *data,
									t_token **tokens, int i);
int								parse_variable_assignment(char *input, int i,
									int start, t_token **tokens);
int								is_dangerous_command(t_cmd_list *cmd_list,
									t_env **env_list);
void							init_heredoc_ctx(t_heredoc_ctx *ctx);
char							*get_base_filename(void);
int								init_ctx(t_heredoc_ctx *ctx,
									const char *base_filename);
int								crecri_file(char *f_name, char *cntnt,
									char *base_path, char *base_f_name);
